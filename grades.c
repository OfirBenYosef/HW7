...
* @returns NULL if there ins't student with that "id".
*/
...
* @returns 100 on error and 0  on success.
*/
...
* @the given student name and id, set num of course an avrege to 0 . 
* @ return 0 on success and 100 if there is an error 
*/
...
/**
* @returns pointer to node which contains 
* @the given course name and the matching grade
* @if a course with the given "name" is already exists in the given list.
...
* @return 0 on success an 0 if fail 
*/
...
void  course_data_destroy (void *elem){
	...
	/*free the name of the course*/
	free((to_be_destroy->course_name));
	/*free the given course_data*/
	free(elem);	
}

void student_data_destroy (void *elem){
	student_data *to_be_destroy=(student_data*)elem;
	/*free the name of the student*/
	free((to_be_destroy->name));
	/*free the courses list*/
	list_destroy((to_be_destroy->student_grades));
	/*free the given student_data*/
	free(elem);
}
/**
 *@ allocates space to a new student_data elememt
 * if success return ptr for the new element to **out and 0
 * else free the allocated space and return 100 (error)
*/
int student_data_clone(void *elem, void **out){
...
/**
 *@ allocates space to a new course_data elememt
 * if success return ptr for the new element to **out and 0
 * else free the allocated space and return 100 (error)
*/
int course_data_clone (void *elem, void **out){
